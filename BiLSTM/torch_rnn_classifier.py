...
class TorchRNNDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(batch):
        ...
        # print(f"Collate_fn: {type(X)}")
        X = torch.nn.utils.rnn.pad_sequence(X, batch_first=True)
        # print(f"Collate_fn padded: {type(X)}")
...
class TorchRNNClassifierModel(nn.Module):
    def __init__(self,
        ...
        #         embedding, len(vocab), self.embed_dim)
        #     self.embed_dim = self.embedding.embedding_dim
        self.srl_embedding = nn.Embedding(len(srl_vocab), self.embed_dim)
        self.rnn = nn.LSTM(
            input_size=100,
    ...
    def forward(self, X, tags, seq_lengths):
        embeds = torch.cat(self.word_embedding(X), self.srl_embedding(tags))
        state = self.rnn_forward(embeds, seq_lengths, self.rnn)
        logits = self.classifier_layer(state)
        return logits

    def rnn_forward(self, X, seq_lengths, rnn):
        # print(f"Rnn_forward: {X.shape()}")
        X = torch.nn.utils.rnn.pad_sequence(X, batch_first=True)
...
class TorchRNNClassifier(TorchModelBase):
    ...
    def fit(self, X, y, **kwargs):
        ...
        # print(f"Before build_dataset: {type(X)}")
        # print(len(X))
        ...
        for iteration in range(1, self.max_iter+1):
            ...
            for X_batch, batch_seq_lengths, y_batch in dataloader:
                ...
                # print(f"In batch, before predict: {type(X_batch)}, {type(X_batch[0])}, {len(X_batch[0])}")
