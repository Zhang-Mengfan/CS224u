...
class NLIAdversaryReader(object):
    def __init__(self,
            src_filename,
    ...
    def read(self):
        ...
        p_id = 1
        category = 'adversary_addamod' if 'ADDAMOD' in self.src_filename else 'adversary_subobjswap'
        for line in open(self.src_filename):
            if (not self.samp_percentage) or random.random() <= self.samp_percentage:
                if 'preprocessed' in self.src_filename:
                    d = json.loads(line)
                else:
                    parts = line.split('\t')
                    d = {
                        ...
                        'gold_label': parts[2].strip(),
                        'category': category,
                        'pairID': f"{p_id}_{os.path.basename(self.src_filename)}",
                        'tags1': None,
                        'tags2': None
                    }
                    p_id += 1
    ...
    def __repr__(self):
        d = {k: v for k, v in self.__dict__.items() if not k.startswith('__')}
        return """"NLIAdversaryReader({})""".format(d)


class AddamodTrainReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/ADDAMOD/add_amod(train).tsv")
        super(AddamodTrainReader, self).__init__(src_filename, **kwargs)


class SubObjTrainReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/SUBOBJSWAP/sub_obj_swap(train).tsv")
        super(SubObjTrainReader, self).__init__(src_filename, **kwargs)


class AddamodDevReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/ADDAMOD/add_amod(dev).tsv")
        super(AddamodDevReader, self).__init__(src_filename, **kwargs)


class SubObjDevReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/SUBOBJSWAP/sub_obj_swap(dev).tsv")
        super(SubObjDevReader, self).__init__(src_filename, **kwargs)


class PreProcAddamodTrainReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/ADDAMOD/preprocessed_add_amod(train).tsv")
        super(PreProcAddamodTrainReader, self).__init__(src_filename, **kwargs)


class PreProcSubObjTrainReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/SUBOBJSWAP/preprocessed_sub_obj_swap(train).tsv")
        super(PreProcSubObjTrainReader, self).__init__(src_filename, **kwargs)


class PreProcAddamodDevReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/ADDAMOD/preprocessed_add_amod(dev).tsv")
        super(PreProcAddamodDevReader, self).__init__(src_filename, **kwargs)


class PreProcSubObjDevReader(NLIAdversaryReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/SUBOBJSWAP/preprocessed_sub_obj_swap(dev).tsv")
        super(PreProcSubObjDevReader, self).__init__(src_filename, **kwargs)
...
class NLIReader(object):
    ...
    def read(self):
        ...
        for line in open(self.src_filename):
            if (not self.samp_percentage) or random.random() <= self.samp_percentage:
                d = json.loads(line)
                if 'preprocessed' not in self.src_filename:
                    d['tags1'] = None
                    d['tags2'] = None
                ex = NLIExample(d)
                if (not self.filter_unlabeled) or ex.gold_label != '-':
...
class SNLITrainReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_1.0_train.jsonl")
        super(SNLITrainReader, self).__init__(src_filename, **kwargs)


class SNLIDevReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_1.0_dev.jsonl")
        super(SNLIDevReader, self).__init__(src_filename, **kwargs)


class SNLITestReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_1.0_test.jsonl")
        super(SNLITestReader, self).__init__(src_filename, **kwargs)


class PreProcSNLITrainReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "preprocessed_snli_1.0_train.jsonl")
        super(PreProcSNLITrainReader, self).__init__(src_filename, **kwargs)


class PreProcSNLIDevReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "preprocessed_snli_1.0_dev.jsonl")
        super(PreProcSNLIDevReader, self).__init__(src_filename, **kwargs)


class PreProcSNLITestReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "preprocessed_snli_1.0_test.jsonl")
        super(PreProcSNLITestReader, self).__init__(src_filename, **kwargs)


class BreakingSNLIReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/dataset.jsonl")
        super(BreakingSNLIReader, self).__init__(src_filename, **kwargs)


class PreProcBreakingSNLIReader(NLIReader):
    def __init__(self, snli_home, **kwargs):
        src_filename = os.path.join(snli_home, "snli_adversaries/preprocessed_dataset.jsonl")
        super(PreProcBreakingSNLIReader, self).__init__(src_filename, **kwargs)
...
def build_dataset(reader, phi, vectorizer=None, vectorize=True):
    ...
    for ex in reader.read():
        if hasattr(ex, 'sentence1_parse'):
            t1 = ex.sentence1_parse
        else:
            t1 = ex.sentence1
        if hasattr(ex, 'sentence2_parse'):
...
def experiment(
    ...
    # Train:
    print("Entering train_func")
    # print(type(X_train))
    # print(len(X_train))
    mod = train_func(X_train, y_train, raw_train)
    # Predictions:
...
def evaluation(
        ...
        verbose=True,
        random_state=None):
    """A function uses trained model to do evaluation

    Parameters
    ----------
    train_reader : `Reader` for testing data.
    mod: Trained model
    phi : feature function
        Any function that takes an `nltk.Tree` instance as input
        and returns a bool/int/float-valued dict as output.
